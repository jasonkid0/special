...
if (isset($_POST['submit'])) {
  ...
  $chk = mysqli_query($db,"SELECT * FROM tbl_schedules_old where section = '$section' AND class_code = '$class_code' AND acad_year = '$acad_year' AND semester = '$semester' AND subj_id = '$subj_id' and time = '$time' and day='$day' and room='$room' ");
  $ct = mysqli_num_rows($chk);
if($ct == 0)
{
   $query = mysqli_query($db,"INSERT INTO tbl_schedules_old (section,class_code,subj_id, faculty_id, day, time, room, acad_year, semester, special_tut) values ('$section','$class_code','$subj_id', '$faculty_id','$day', '$time', '$room', '$acad_year', '$semester', '$special_tut')")or die (mysqli_error($db));
    if($query == true)
    {
...
<?php 
$q = mysqli_query($db,"SELECT *, tbl_semesters.sem_id,tbl_year_levels.year_id,tbl_courses.course_id,tbl_courses.department_id,tbl_departments.department_id FROM tbl_subjects
      ...
      LEFT JOIN tbl_year_levels ON tbl_year_levels.year_id = tbl_subjects.year_id where subj_id = '".$_GET['subj_id']."'")or die(mysqli_error($db));
