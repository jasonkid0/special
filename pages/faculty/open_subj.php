...
if (isset($_POST['submit'])) {
  ...
  $chk = mysqli_query($db,"SELECT * FROM tbl_schedules where class_code = '$class_code' AND acad_year = '$acad_year' AND semester = '$semester' AND subj_id = '$subj_id'");
    $ct = mysqli_num_rows($chk);
if($ct == 0)
{
   $query = mysqli_query($db,"INSERT INTO tbl_schedules (class_code,subj_id, faculty_id, day, time, room, acad_year, semester) values ('$class_code','$subj_id', '$faculty_id','$day', '$time', '$room', '$acad_year', '$semester')")or die (mysqli_error($db));
    if($query == true)
    {
              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <fieldset style="border: 1px solid;padding: 0px 10px;border-color: #d2d2d2">
                    <legend style="text-align:center">Add Petitioned Subject/s (New Curr)</legend>
                    <hr>
                    <div class="row">
                      <div class="col-md-4">
                        ...
                        <div class="form-group">
                          <select name="subj_id" id="status" data-style="btn btn-primary btn-round" required class="form-control select-2">
                            <option selected disabled>Select Subject</option>
                              <?php 
                                $q1 = mysqli_query($db,"SELECT *,tbl_departments.department_id from tbl_subjects_new 
